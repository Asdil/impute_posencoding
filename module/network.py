...
class Generator(nn.Module):
    ...
    def _build_conv_submodule(self):
        ...
        for i in range(len(self.args.conv_channel_list)-1):
            ...
            conv_sequential.add_module('activation_{}'.format(i), nn.Sigmoid())
    ...
    def _build_linear_submodule(self):
        ...
        for i in range(len(self.args.linear_size)):
            ...
            linear_sequential.add_module('activation_{}'.format(i), nn.Sigmoid())
    ...
    def _conv_forward(self, x, z, m, pos_encoding):
        inp = m * x + (1 - m) * z + pos_encoding
    ...
    def _linear_forward(self, x, z, m, pos_encoding):
        inp = m * x + (1-m) * z + pos_encoding
        inp = torch.cat((inp, m), dim=1)
        out = self.linear_sequential(inp)
        return out

    def forward(self, x, z, m, rate, pos_encoding):
        if self.args.gconnection == 'linear':
            out = self._linear_forward(x, z, m, pos_encoding)
            return self.sigmoid(out)
        elif self.args.gconnection == 'conv':
            out =  self._conv_forward(x, z, m, pos_encoding)
            return self.sigmoid(out)
        elif self.args.gconnection == 'linear-conv':
            rate = torch.from_numpy(np.repeat(rate[np.newaxis,...], x.size()[0], axis=0)).float()
            conv_out = self._conv_forward(x, z, m, pos_encoding)
            linear_out = self._linear_forward(x, z, m, pos_encoding)
            final_inp = (conv_out + linear_out) * self.w1 + rate * (1 - self.w1)
            final_out = self.sigmoid(self.final_fc(final_inp))
            return final_out
        else:
