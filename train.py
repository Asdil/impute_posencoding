...
def run(args):
    ...
    pos = np.array([p for p in range(500)])/500
    pos_encoding = torch.tensor(pos).float()
    ...
    for epoch in range(args.num_epochs+1):
        ...
        with torch.set_grad_enabled(True):
            ...
            for _ in range(2):
                ...
                # Train D
                G_sample = netG(X_mb, New_X_mb, M_mb, rare, pos_encoding)
            ...
            D_prob = netD(X_mb, M_mb, G_sample, H_mb)
            D_prob.detach_()
            G_loss1 = ((1 - M_mb) * (torch.sigmoid(D_prob)+1e-8).log()).mean()
            # MSE Loss
            loss_rare = np.repeat(rare[np.newaxis,...], args.batch_size, axis=0)
            loss_rare = torch.FloatTensor(loss_rare)
            G_mse_loss = mse_loss(X_mb, G_sample).mean() / M_mb.sum()
            G_loss = G_loss1 + args.alpha*G_mse_loss + args.beta * torch.nn.L1Loss()(torch.mean(G_sample*(1-M_mb), dim=0),torch.FloatTensor(rare))
            optimG.zero_grad()
            G_loss.backward()
            optimG.step()
        ...
        if epoch % args.print_freq == 0:
            ...
            with torch.set_grad_enabled(False):
                ...
                samples2 = netG(X_mb, New_X_mb, M_mb, rare, pos_encoding)
